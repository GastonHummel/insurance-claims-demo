...
var CtaButtons = {
    ...
    buttonClick: function(buttonName) {
        ...
        if (button) {
            ...
            } else if (button.hasOwnProperty('chat')) {
              if (VideoPlayerInterface.iframeWindow) {
                VideoPlayerInterface.actions.pause();
                VideoPlayerInterface.iframeWindow.rtc.utils.track('sidebutton.click', buttonName);
              }
              $('#cvChatContainer').find('#cvMessages').empty();
              $('#cvChatContainer').removeClass('cv--is-hidden cv--is-closed').addClass('cv--is-open');
              $('#cvChatContainer').attr('aria-expanded', true);
              $('#cvChatContainer').find('#cvMessageTextarea').focus();
            } else if (button.hasOwnProperty('url')) {
              if (VideoPlayerInterface.iframeWindow) {
                VideoPlayerInterface.actions.pause();
                VideoPlayerInterface.iframeWindow.rtc.utils.track('sidebutton.click', buttonName);
...
var iFrameCommunication = function() {
    ...
    function postMessageCommunication() {
        window.addEventListener("message", function (e) {
            ...
            switch (e.data.message) {
                case messagesToHandle.showCustomError:
                    iFrameEvents.showErrorCard(e.data.data);
                    break;
                case messagesToHandle.closeLowBandwidthCard:
    ...
    /**
     * Find Js code injection
     *
     * @param text source
     */
    function isJavaScriptInjected(text) {
        return text.match(/<script[\s\S]*?>[\s\S]*?<\/script>/gi, "") || text.match(/(on[a-z]+=(["']).+?\2)/gi, "");
    }

    /**
     * Check if origin is invalid or if send data contain js
     *
     * @param event object send in event
     * @returns {boolean}
     */
    function isEventInvalid(event) {
        return !isOriginValid(event.origin) || typeof event.data.data !== "string" || isJavaScriptInjected(event.data.data);
    }

    /**
     * Check if origin is valid
     *
     * @param eventOrigin origin from event
     * @returns {boolean}
     */
    function isOriginValid(eventOrigin) {
        return getDocumentReferrer() === eventOrigin && getLocationOrigin() === eventOrigin;
    }

    /**
     * Getting location origin
     *
     * @returns {string}
     */
    ...
    function getDocumentReferrer() {
        ...
        return String(document.referrer).replace(/(https?:\/\/[^\/]+).+/, "$1");
    }
