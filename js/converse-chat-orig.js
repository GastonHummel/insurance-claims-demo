...
function ConverseWebClient(options) {
    ...
    this.showMinimizeButton = !!options.showMinimizeButton;
    ...
    this.minimizeCallback = options.minimizeCallback;
	...
	this.serverUserImage = options.serverUserImage || "eo-square-mark.png";
...
ConverseWebClient.init = function(opts, callback) {
	...
	client._init(callback);

	return client;
};

ConverseWebClient.prototype._init = function(callback) {
	var client = this,
		url = client.host + "/chat/init",
		chatId = ConverseUtil.localStorage(client.botId + "-chatid"),
		queries = [
			"tenant=" + encodeURIComponent(client.tenant),
			"bot_id=" + encodeURIComponent(client.botId)
		];

	if (chatId) {
		queries.push("chat_id=" + chatId);
	}

	if (client.showHistory === false) {
		queries.push("show_history=false");
	}
	...
	$.ajax({
		...
		success: function(res) {
			...
			client.renderMain.call(client, typeof callback === "function" ? callback.bind(client, client) : null);
...
ConverseWebClient.prototype._buildInterface = function() {
    ...
    if (client.showMinimizeButton) {
        $btnGroup.append($(
            "<button data-widget=\"minimize\" " +
                    "id=\"minimizeChat\" " +
                    "class=\"pb-chat-header-button pull-right\"" +
                    "type=\"button\">" +
                "<i class=\"pb-chat-minimize-icon fas " + client.minimizeIcon + "\"></i>" +
                "<i class=\"pb-chat-maximize-icon fas " + client.maximizeIcon + "\"></i>" +
            "</button>"));
    }
...
ConverseWebClient.prototype.onConnect = function() {
	...
	if ((!client.showHistory || client.history.length === 0) && !client.tryReconnect && client.startPhrase) {
		client.sendStartPhrase(client.startPhrase);
	}

	client.pingInterval = setInterval(client.ping.bind(client), 30000);
...
ConverseWebClient.prototype.clearChat = function() {
	...
	$(this.baseContainer).find(".pb-dropdown-content").removeClass("show");
	this.lastTimestamp = 0;
...
ConverseWebClient.prototype.onSystemMessage = function(message) {
	switch (message.message.text) {
		case "#agentDisconnected#":

			this.events.onAgentDisconnected(message);

			break;
		case "#agentReconnected#":

			this.events.onAgentReconnected(message);
...
ConverseWebClient.prototype.renderMain = function(callback, history) {
    ...
    $(me.baseContainer).find("#minimizeChat").click(function (event) {
	...
	$(me.baseContainer).find("#closeChat").click(function (event) {
        event.preventDefault();
        event.stopPropagation();
	...
	$(me.baseContainer).find("#status_message").keydown(function (event) {
		if (event.which === 13 && me.connected) {
			me.messageSend();
			event.preventDefault();
		}
    ...
    $(me.baseContainer).find(".pb-chat-popup-head").click(function onClickHeader(event) {
        if ($(me.baseContainer).hasClass("pb-chat-popup-minimize")) {
            event.preventDefault();
            event.stopPropagation();

            $(me.baseContainer).removeClass("pb-chat-popup-minimize");
...
ConverseWebClient.prototype.renderHistory = function(history) {
	let events = [
		"text",
		"postback",
		"buttons",
	...
	if (history.length) {
		history = history.sort(function sortHistory(a, b) {
		...
		for (let i = 0; i < history.length; i++) {
			...
			switch (message.data.type) {
				...
				case "buttons":
					...
					break;
				case "card":
					this.addCardMessage(message.data.data);
					break;
				case "buttonCard":
					this.onCardButton({
						direction: message.direction,
...
ConverseWebClient.prototype.sendStartPhrase = function(payload) {
	var self = this;
	var msg = {
		text: payload,
		type: "start"
...
ConverseWebClient.prototype.show = function() {
    ...
    if (this.startMinimized) {
        $(this.baseContainer).addClass("pb-chat-popup-minimize");
    }
};

ConverseWebClient.prototype.minimize = function() {
    $(this.baseContainer).toggleClass("pb-chat-popup-minimize");

    if (this.minimizeCallback && $(this.baseContainer).hasClass("pb-chat-popup-minimize")) {
...
ConverseWebClient.prototype.setWrapperItems = function(data) {
	...
	if (!this.lastContainer || this.lastDirection !== data.direction || reset) {
		...
		if (data.direction === "inbound") {
			$msgWrapper.append($("<img alt=\"Precisely\" src=\"" + this.serverUserImage + "\" class=\"pb-chat-direct-chat-img\">"));
		}

		$msgWrapper.append($items);

		if (data.direction === "outbound" && !!this.clientUserImage) {
			$msgWrapper.append($("<img alt=\"Precisely\" src=\"" + this.clientUserImage + "\" class=\"pb-chat-direct-chat-img\">"));
		}
...
/**
 * Sends a ping message to the websocket to keep the connection open. This prevents the browser from
